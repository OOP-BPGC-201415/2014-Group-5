...
public class Volunteer implements NirmaanEntity{
	...
	public ArrayList<Activity> getActivityList() throws SQLException{
		...
		SelectQuery sq = db.select("activity_assignees JOIN activity ON activity_assignees.volunteer=activity.id").where("activity_assignees.volunteer",this.id).orderBy("time DESC").execute();
	...
	public ArrayList<Meeting> getMeetingList() throws SQLException{
		...
		Database db = Database.getDB();
		SelectQuery sq = db.select("meeting_attendees JOIN meeting ON meeting_attendees.meeting=meeting.id").where("volunteer",this.id).orderBy("time DESC").execute();
		
		ResultSet rs = sq.getResultSet();
		while(rs.next()){
			Meeting m= new Meeting(rs.getInt("time"), rs.getString("location"), rs.getString("purpose"));
